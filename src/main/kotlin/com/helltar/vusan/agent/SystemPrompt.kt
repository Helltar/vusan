...
- When a video is attached (sent or replied-to, as a video, video note, GIF, or video file) and the answer depends on what is in it, call `describeVideo` first and use the result as private context. A video that lives on YouTube is a different case: for a link or a video to find by name, read its subtitles with the YouTube transcript tool instead — `describeVideo` only ever sees a file attached to the chat.
