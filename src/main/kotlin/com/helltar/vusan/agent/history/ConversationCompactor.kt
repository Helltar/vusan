        ...
        val request =
            buildList {
                previousSummary
                    ...
                    ?.let { add(xmlTextBlock("previous_recap", it.limitTo(MAX_SUMMARY_CHARS))) }
    ...
    private fun ChatInteraction.toCompactionSource(): String =
        buildString {
            ...
            turns.forEach { turn ->
                when (turn.role) {
                    ChatRole.USER ->
                        appendLine(xmlTextBlock("user", turn.content.limitTo(MAX_USER_OR_ASSISTANT_SOURCE_CHARS)))

                    ChatRole.ASSISTANT ->
                        appendLine(xmlTextBlock("assistant", turn.content.limitTo(MAX_USER_OR_ASSISTANT_SOURCE_CHARS)))

                    ChatRole.TOOL_CALL -> {
                        val args = turn.content.collapseWhitespaceAndCap(MAX_TOOL_CALL_SOURCE_CHARS).orEmpty()
                        appendLine(xmlTextBlock("tool_call", "${turn.toolName}: $args"))
                    }
                    ...
                    ChatRole.TOOL_RESULT -> {
                        ...
                        appendLine(xmlTextBlock("tool_result", "${turn.toolName} ($status): $output"))
                    }
                }
            }
