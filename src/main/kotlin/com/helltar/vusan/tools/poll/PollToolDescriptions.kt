...
internal object PollToolDescriptions {
    ...
    const val CREATE_POLL =
        "Creates a Telegram poll (regular vote, no correct answer) with answer options. " +
                ...
                "Generate the question and options yourself if needed, keep options concise, and after calling this tool write a short natural comment for the user; the poll will be sent automatically."

    const val QUESTION =
        "Poll question shown in Telegram. " +
                "Keep it short and clear."

    const val OPTIONS =
        "Answer options for the poll. " +
                "Provide 2 to 10 items."

    const val IS_ANONYMOUS =
        "Whether Telegram should hide who voted. " +
                "Default `true`, matching Telegram's normal poll behavior. " +
