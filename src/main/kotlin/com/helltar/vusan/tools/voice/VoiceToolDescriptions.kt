...
internal object VoiceToolDescriptions {
    ...
    // shared with the round video message: both speak the text through the same model, so the rules for
    // directing that voice must not drift apart between the two tools.
    const val AUDIO_TAG_RULES =
        "By default, send plain prose with NO audio tags — the default voice already sounds natural and unwanted tags make it theatrical. " +
                """Add `eleven_v3` audio tags in square brackets only when the user explicitly directs the delivery: asks for a manner ("say it quietly", "like a pirate", "sarcastically"), scripts the line themselves (`start quietly with "hi", then loudly "how are you?"`), or asks for a clearly performative read (poem, dialogue, dramatic scene). """ +
                "When tags are warranted, the set is open-ended — invent whatever bracketed cue fits (emotion, volume, non-verbal reactions, pacing, emphasis, accents, character voices, sound effects). " +
                "Layer freely (`[British accent][exasperated]`), place each tag right before the words it should color, and keep tag names in English even when the spoken text is in another language. " +
