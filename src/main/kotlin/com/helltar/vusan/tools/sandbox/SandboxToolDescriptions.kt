...
internal object SandboxToolDescriptions {
    ...
    const val CODE_EXECUTION =
        "Executes Python 3 code in an isolated sandbox to compute an exact answer or produce a chart or file, then returns its output. " +
                ...
                "When the user attaches a file (CSV, JSON, text, image, `.docx`, `.pdf`, etc.), it is placed in the working directory; an `<attached_file>` note gives its exact name. Read it directly from your script (for example `pandas.read_csv(\"data.csv\")` or `docx.Document(\"input.docx\")`) instead of asking the user to paste the contents. " +
                "Use it to create or edit Word and PDF documents on request: build a `.docx` with `python-docx` or a `.pdf` with `fpdf2`, read or edit an existing one with `python-docx`/`pypdf`. " +
                ...
                "Available libraries: `numpy`, `pandas`, `matplotlib`, `sympy`, `scipy`, `Pillow` (PIL), `python-docx` (import `docx`), `fpdf2` (import `fpdf`), `pypdf`. " +
    ...
    const val CODE_EXECUTION_SOURCE =
        "A complete, self-contained Python 3 script. " +
                ...
                "To produce a Word or PDF document, save it with `python-docx`/`fpdf2` under a descriptive new filename such as `report.docx` or `report.pdf`; to edit an attached document, open it (`docx.Document(\"input.docx\")` or `pypdf.PdfReader(\"input.pdf\")`) and save the result under a new filename. " +
                ...
                "Emoji in a single-line `draw.text(...)` call are handled automatically and rendered in monochrome (black-and-white only; color emoji are not available), so you can put emoji directly in the string. " +
