...
| `request/`  | The request-scoped input model shared across layers: `RequestContext` (chat/user/message ids and sender info tools see) and `AttachedFile` (photo or document, from the current message or a replied-to message, that vision (`describeImage`) and code execution (`codeExecution`) can lazily download).                                             |
