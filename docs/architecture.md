...
| Web search picks the wrong provider, or results are thin                        | the `@LLMDescription` text that ranks them: `tools/searxng/SearxngToolDescriptions.kt` (`metaSearch`, the default) and `tools/tavily/TavilyToolDescriptions.kt` (`webSearch`, for longer extracts)                                                                                                    |
